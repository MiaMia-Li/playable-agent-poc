...
export const PLAYABLE_TOOLS_PROMPT =
  'Read sandbox-tools.json for detected commands. Use node assets/starter/work/node-tools.mjs header|json|hash INPUT OUTPUT for binary headers, JSON and hashes; do not assume xxd, jq or Python exist. Before browser acceptance, check whether /opt/playable-tools/README.md exists. If present, read it and use the preinstalled Playwright wrapper; do not run npm install or download browsers again. If absent, use the available browser tooling. Cocos inspection exit zero means the diagnostic report was saved, not that decoding is supported. Read status and code in work/cocos-inspection/latest.json directly; on unsupported, inspect the report and switch to targeted source inspection without retrying unchanged input. Do not depend on a temporary pretty report or execute sliced obfuscated initialization.'
