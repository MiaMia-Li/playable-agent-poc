        ...
        .array(
          requirementToolCallSchema.extend({
            ...
            confirmation: generatedConfirmationProposalSchema
              .safeExtend({
                // 严格模型输出必须包含该键，但允许 null 表示交给宿主采用默认基底。
                baseline: generatedConfirmationProposalSchema.shape.baseline.unwrap(),
...
function validateConfirmationAlignment(brief: RequirementBrief, value: unknown, sourceHtmlAssetId?: string) {
  ...
  // 上传基底的 ID 随后由宿主对本任务的 HTML 附件校验，不能直接用于构建。
  const confirmation = confirmationProposalSchema.parse(
    (sourceHtmlAssetId || generated.baseline?.kind === 'uploaded_html') && generated.rendering?.renderer === 'template'
...
export const REQUIREMENT_AGENT_INSTRUCTIONS = [
  ...
  'attachedAssetIds lists the latest message attachments. referenceImages lists available image attachments from the conversation, including earlier messages. You may inspect images from either set when relevant to the current request. Other uploadedAssets are metadata only and are not automatically current requirements.',
  ...
  'Use inspect_uploaded_assets when uploaded asset metadata affects the plan. This is a metadata-only operation inside the terminal plan, not an analysis step; it must be followed by a valid terminal call in that same plan.',
  'Uploaded file metadata is not evidence of file contents. Claim to have inspected content only when supplied source content or completed analysis tool results support it. Never invent unavailable analysis capabilities. When the user asks to apply uploaded resources, record their asset source and intended use from the conversation; treat filename-based interpretations as tentative. If the intended use is unclear or requested analysis is unavailable, explain the limitation and ask a focused question through the normal requirement workflow instead of ending with preparatory tools.',
  ...
  'When currentArtifact.hasArtifact is true, never call submit_confirmation. For a clear change request, call list_playable_capabilities and then submit_revision with the complete updated confirmation plus a concise revision plan. The existing validated route may be reused without another validate_implementation_route call when the revision does not change the core gameplay or route. Use patch for scoped changes that should preserve the current implementation. Use regenerate for a requested broad redesign or core structural change. Dissatisfaction alone or uploading another file does not request regeneration or a baseline switch. The revision plan must say what changes and what stays unchanged.',
  ...
  'When currentArtifact.lockedRevisionBase is present, the user manually locked the revision baseline. This overrides version references in conversation and pendingRevision. Its source has already been read by the server, and currentConfirmation is its saved configuration. Use that configuration and source as the baseline, set confirmation.baseline to its exact version/buildId, and set requestedBaseVersion to its version. Choose patch or regenerate independently according to the requested scope. Do not switch to another source. Apply only the new requested changes; older conversation changes are not automatically carried forward.',
  'referenceImages contains ordinary image attachments, not preclassified screenshots. Infer each image use from the user request and conversation, inspecting relevant files with inspect_reference_images. For example, use an uploaded logo as an original asset when asked to place it, inspect a reported defect when asked to fix it, and use a layout reference when asked to match it. Do not infer a problem, target or source version from the existence of a previous build. Do not ask users to classify uploads when intent is clear. Descriptions record the message that attached the image; later instructions may change its use. When an image is used in a resource field, set status 用户上传 and include its exact filename and intended use in treatment; the host binds the original file. Images used only for visual discussion need no resource binding. Preserve existing assets unless the user asks to replace them. Record intended image usage in the brief and confirmation or revision, and ask a focused question only when context cannot establish it.',
