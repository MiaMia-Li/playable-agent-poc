...
Read playable HTML with `node assets/starter/work/node-tools.mjs slice FILE FROM TO` and find text in it with `node assets/starter/work/node-tools.mjs search FILE PATTERN`. Those files embed their assets, so one line can be megabytes; `cat`, `sed`, `nl`, `head`, `tail` and unbounded `rg` on them return a capped, useless dump. The bytes one command hands back are capped: a capped-output notice means narrow the range, not retry the same command.
