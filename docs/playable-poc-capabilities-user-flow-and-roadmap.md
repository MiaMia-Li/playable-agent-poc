...
- 需求 Agent 会先识别搜索意图：用户明确要求时立即搜索；认为搜索可能有帮助时先请求一键确认；需求明确、有强参考素材、修改已有版本或普通问答时跳过。
- 独立市场研究 Agent 使用 OpenAI Responses Web Search，只检索 TikTok Creative Center、Google Ads Transparency、Meta、AppLovin 和 Liftoff 的受控公开域名。
- 搜索分为发现和深度分析两阶段，返回 3–5 个结构化候选，包括核心循环、开场钩子、交互、反馈、CTA、来源证据、可信度、局限、可借鉴亮点和排除元素。
- 公开趋势不会被包装为 CTR、CVR、IPM、ROAS 或其他真实投放表现；研究卡片明确展示该限制。
- 研究进度和结果全部显示在左侧用户对话流中，不占用右侧 Preview。
- 研究运行、候选和不可变选择独立持久化；相同用户和规范化 Search Brief 在 24 小时内可复用缓存。
- 搜索完成不会修改 Requirement Brief 或任务阶段。只有用户采用主方向、组合亮点或行业总结后，服务端才根据持久化 ID 校验并把解析结果交给下一轮需求 Agent。
- 本地 Demo 使用固定三候选数据和同一套 Schema，不发起网络请求。
...
- 商店地址必须是绝对 HTTPS URL。
- 当前交付参数固定为 AppLovin、逻辑画布 `360 × 640`、响应式展示、单文件 HTML、包体不超过 10 MiB。
- 构建时会将确认方案投影为五组生产配置：
...
- HTML 包体超过 Plugin 声明的 10 MiB 上限时标记为渠道交付不合规，但保留可用产物。
...
6. 如果用户明确要求市场搜索，Agent 立即返回对话内研究卡片；如果只是可能有帮助，先请求用户确认。用户可以采用主方向、跨候选组合亮点、仅采用行业总结，或直接跳过。
7. 信息不足时，Agent 动态请求文本、单选、多选、链接或审批输入；用户继续补充。
8. 信息完整后，Agent 选择最合适的生产路径，并返回：
   - 精确匹配、近似匹配或自由生成。
   - 路由置信度。
   - 近似或自由生成方案与模板的差异。
   - 完整确认配置。
9. 用户检查并修改确认表，选择每个生产素材槽使用内置默认或本地上传，也可以补充参考图片/视频；AI 生成按钮置灰。
10. 用户点击本地上传并选择文件，服务端完成归属、逐槽类型和大小校验；参考附件仅保存元数据供 Agent 识别，不进入最终产物。
11. 用户点击确认构建；如果存在待上传/待生成素材、无效 URL 或缺少所需 API Key，系统停留在确认阶段并提示处理。
12. 服务端认领当前版本并准备用户上传素材。
13. 系统创建隔离 Sandbox，复制只读 Plugin、确认配置和任务素材。
14. Codex 在任务工作区执行构建，随后运行行为和安全检查。
15. 全部门禁通过后，系统保存四件套产物和成功版本记录，任务直接进入 `ready`。
16. 用户在 Preview 中切换版本、横竖屏、刷新、控制静音，并检查视觉、节奏、易理解性和品牌一致性。
17. 用户可以立即下载 HTML、生产配置、素材清单和验证报告，也可以继续对话修改下一版方案。
