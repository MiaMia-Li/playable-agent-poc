...
describe('versions page loading', () => {
  ...
  it('loads build records once without embedded previews and links previews to a new tab', async () => {
    ...
    expect(screen.getByRole('columnheader', { name: '游戏标题' })).toBeInTheDocument()
    expect(screen.queryByRole('columnheader', { name: '构建 ID' })).not.toBeInTheDocument()
    expect(within(firstVersion).queryByText('需求 1')).not.toBeInTheDocument()
    expect(within(firstVersion).getByText('测试游戏')).toBeInTheDocument()
    expect(within(firstVersion).getByText('build-1')).toHaveClass('text-xs', 'text-muted-foreground')
    expect(within(firstVersion).getByText('1.0 MB')).toBeInTheDocument()
    expect(within(firstVersion).getByText('校验通过')).toBeInTheDocument()
    expect(within(firstVersion).queryByText('下落补位')).not.toBeInTheDocument()
    ...
    expect(within(screen.getByRole('dialog', { name: '构建配置' })).queryByText('下落补位')).not.toBeInTheDocument()
