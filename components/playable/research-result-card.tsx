...
export function ResearchResultCard({
  ...
  if (adoptedSelection) {
    return (
      <section aria-label="市场参考分析" className="mt-3 border-l-2 border-primary/30 pl-3">
  ...
  return (
    <section aria-label="市场参考分析" className="mt-3 space-y-3">
      <header className="space-y-1.5">
        <div className="flex items-center gap-2">
          <Search className="text-primary size-4" aria-hidden="true" />
          <h2 className="font-semibold">市场参考分析</h2>
          <span className="text-muted-foreground text-xs">{report.candidates.length} 个方向</span>
        </div>
        <p className="text-muted-foreground text-xs leading-5">
          公开案例只用于分析玩法趋势，不代表真实 CTR、CVR、IPM 或 ROAS。
        </p>
        {report.industrySummary.trends.length > 0 && (
          <p className="text-muted-foreground text-xs leading-5">
            <span className="text-foreground font-medium">趋势：</span>
            {report.industrySummary.trends.join('、')}
          </p>
        )}
      </header>
      ...
      <RadioGroup
        ...
        onValueChange={setPrimaryCandidateId}
        className="divide-y border-y"
      >
        {report.candidates.map((candidate, index) => (
          <article key={candidate.id} className="py-3">
            ...
            <details className="group mt-2 ml-7">
              <summary className="text-muted-foreground hover:text-foreground flex w-fit cursor-pointer list-none items-center gap-1 text-xs transition-colors [&::-webkit-details-marker]:hidden">
                查看玩法细节
                <ChevronDown className="size-3.5 transition-transform group-open:rotate-180" aria-hidden="true" />
              </summary>
              <div className="mt-3 space-y-3 border-l pl-3 text-xs">
                <dl className="space-y-3">
                  <div>
                    <dt className="font-medium">开场钩子</dt>
                    <dd className="text-muted-foreground mt-0.5 leading-5">{candidate.openingHook}</dd>
                  </div>
                  <div>
                    <dt className="font-medium">反馈与 CTA</dt>
                    <dd className="text-muted-foreground mt-0.5 leading-5">
                      {candidate.feedback} {candidate.cta}
                    </dd>
                  </div>
                </dl>
                <div className="space-y-2">
                  <p className="font-medium">可组合亮点</p>
                  <div className="space-y-2">
                    {candidate.borrowableHighlights.map((value) => {
                      ...
                      return (
                        <label key={value} className="flex cursor-pointer items-start gap-2 leading-5">
                          <Checkbox
                            aria-label={value}
                            checked={checked}
                            disabled={disabled || submitting}
                            onCheckedChange={(next) =>
                              setSelectedHighlights((highlights) =>
                                next
                                  ...
                                  : highlights.filter(
                                      (highlight) =>
                                        highlight.candidateId !== candidate.id || highlight.value !== value,
                                    ),
                              )
                            }
                          />
